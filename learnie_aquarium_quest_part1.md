...
| マイク   | **デフォルト OFF（ミュート）**。4択表示中はマイクロック（ボタンで答える）                              |
| 4択の並び | **毎回ランダム**（正解が常に左上にならない）。Ch4 色ボタンはスピーカーなし                              |
| 不正解   | ランダムなソフトリトライ（例: `Nice try!` / `So close!` / `Oops!` / `ざんねん！` / `ちがうみたい！`）— **同じ文を毎回繰り返さない**。**正解は言わない**。同じビートの4択を再表示 |
| バッジ   | チャプター／フリートーク／**いっぱつせいかい**（初回クリック正解）。章やりなおしで初回正解を更新可。受け取りは「うけとる」        |
| 章のつなぎ | 「つぎの章へ…」ローディング。**Learny が次章の発話を始めるまで 4択は出さない**（Ch2→Ch3 も含む Live 再接続） |
| 自由会話中 | **4択を隠す**（Ch2 ③④・さがし待ちなど）。Ch4「すきないろは？」は **色ボタンあり**（自由入力ではない）           |
...
| Chapter 4「すきないろは？」           | **あり**（色ボタンのみ・下記6色）       |
| Chapter 4「I made ___ glass!」 | **あり**（色ボタンのあと make+tell 合体ライン） |
| Daily English                | **なし**                   |
| Chapter 3 / 5 / 6 / Final    | **あり**                   |
| Ending                       | なし（会話ビート→フリートーク）          |
...
**不正解時（各ビート共通）:**（毎回ちがう言い方 — 固定の Almost! / おしい！もういちど！だけを繰り返さない）
...
**Learny（必ずこの全文・モーラどおり）:**  
`くいずたいむ！「がらすが ひつよう」は えいごで？`  
FORBIDDEN 短縮: `くいずたいむ！は英語で？`（「がらすが ひつよう」を落とす）  

画面の「答えをタップ」タイトル右の再生ボタンで、もんだいを もういちど きける。 
...
**流れ:** Beat A1（色ボタン）→ **Beat A2+B 合体**（静的音声＋ガラス MCQ）

## Beat A1 — すきないろ（色ボタンのみ）

章オープニング（Mini quiz 完了直後）— **このセリフ固定**:  
...
| ボタン |
| --- |
| orange / red / blue / green / yellow / pink |

- **ボタンタップだけ**が `favoriteColor` を記録する  
- **音声・テキスト入力は受け付けない**（「すきな色はボタンで選んでね。」）  
- レッスン外の色フォールバック（`Which colour would you pick…` / `この中だったらどの色がすき？`）は **使わない**  
- **ここでガラス MCQ は出さない / Chapter 5 へ進まない**

## Beat A2+B — その色のガラスを作ろう＋できたらいってね（1ターン）

色ボタンの直後（クライアントが1回だけ配信）:  
**Learny（1メッセージ）:**  
`Let's make orange coloured glass! Tell me when you make one!`  
`おれんじいろの がらすを つくろう！つくれたら「おれんじいろの がらすを つくった！」って えいごで おしえてね！`  
（色は選んだボタンに合わせて変える）  
→ **このあと ガラス用 4択 MCQ を表示**
...
※ 色を聞いたあとに `What's your favorite color?` を聞き直さない
...
Live セッションは **短い Daily English 専用プロンプト**（章・クイズ・MCQ スキャフォールドなし）を使う。  
長い全文レッスン指示は返事が遅くなるため使わない。
...
子ども1ターンごとに：具体反応 → **フォローアップ質問は1つだけ**。  
**毎回 英語のあとにマッチするひらがな**（英語だけの返答は禁止。英語のやり直し禁止）。  
...
- 正解か不正解か
- 何回目の挑戦か（章プレイ回数つき）
- 初回クリックかどうか（いっぱつせいかい用）
- 時刻

集計：生徒ごとの ○正解 / ×不正解 / 総回答数、アクティビティタイムラインに「4択正解」「4択不正解」  
**いっぱつせいかい:** 各ビートで、どのプレイでも「そのプレイの最初の1クリックが正解」なら加点。全ビート達成でゴールド。章のやりなおしで更新可。